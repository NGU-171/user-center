...
import { useModel } from 'umi'; // history, 先不加
...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (values: API.LoginResult) => {
    try {
      // 注册
      const user = await register({ ...values, type });
          ...
          {type === 'account' && (
            <>
              <ProFormText
                ...
                placeholder={'用户账号: admin or user'}
                rules={[
                  {
                    required: true,
                    message: '用户名是必填项！',
